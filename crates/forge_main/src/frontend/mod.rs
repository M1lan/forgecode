...
//!   version constant.
//! - [`console_writer::JsonConsoleWriter`] — `ConsoleWriter` adapter that wraps
//!   streaming-renderer byte writes into `chunk` events.
//! - [`orchestrator::JsonFrontend`] — high-level event-emit API the UI
//!   lifecycle calls into. One per process; held as `Option<Arc<JsonFrontend>>`
//!   on the [`crate::ui::UI`] struct.
//! - [`router::EventRouter`] — background stdin-reader thread that
//!   demultiplexes [`protocol::ClientEvent`]s into prompt-loop events and
//!   selector responses.
//! - [`selector_backend::JsonSelectorBackend`] —
//!   [`forge_select::SelectorBackend`] implementation that emits `select`
//!   events and blocks on matching `select_response`s.
//!
