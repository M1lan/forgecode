...
<!-- Regenerate with `just docs`; `just docs-check` gates it in `just verify`. -->
...
| `ci` | – | What GitHub actually gates on, reproduced locally (coverage + benchmark). |
...
| `pre-push` | – | Fast gate for a commit loop. |
| `verify-clean-tree` | – | Fail if any source file changed during a build/test run. |
| `verify` | – | Full pre-push gate. Every step fails loud; none can skip silently. |
...
| `fix` | – | Everything that writes: format, clippy fixes, typo fixes. |
...
| recipe | arguments | what it does |
|---|---|---|
| `docs-check` | – | fail if docs/justfile.md has drifted from this file; part of `verify` |
